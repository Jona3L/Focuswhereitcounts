...
def main(args):
    ...
    for idx, img_path in enumerate(image_list):
        ...
        # Build the final prompt so that <image> always appears first
        # Remove <image> if the user already included it
        user_prompt = args.prompt.replace("<image>", "").strip()
        # Prepend <image> to ensure the image is first, then the user's text prompt
        gen_prompt = f"<image>\n{user_prompt}"

        # Tokenize the prompt (the helper function replaces <image> token with IMAGE_TOKEN_INDEX)
